...
// ─── Tax Data ─────────────────────────────────────────────────────────────────
const TAX_DATA = {
  UK: { currency:"GBP", symbol:"£",  name:"United Kingdom",       flag:"🇬🇧", personalAllowance:12570, bands:[{min:0,max:12570,rate:0},{min:12570,max:50270,rate:.20},{min:50270,max:125140,rate:.40},{min:125140,max:Infinity,rate:.45}], ni:{threshold:12570,upperLimit:50270,lowerRate:.08,upperRate:.02} },
  US: { currency:"USD", symbol:"$",  name:"United States",        flag:"🇺🇸", standardDeduction:14600, bands:[{min:0,max:11925,rate:.10},{min:11925,max:48475,rate:.12},{min:48475,max:103350,rate:.22},{min:103350,max:197300,rate:.24},{min:197300,max:250525,rate:.32},{min:250525,max:626350,rate:.35},{min:626350,max:Infinity,rate:.37}], fica:{socialSecurityRate:.062,socialSecurityCap:168600,medicareRate:.0145,additionalMedicareRate:.009,additionalMedicareThreshold:200000} },
  CA: { currency:"CAD", symbol:"CA$",name:"Canada",               flag:"🇨🇦", personalAmount:15705, bands:[{min:0,max:57375,rate:.145},{min:57375,max:114750,rate:.205},{min:114750,max:158519,rate:.26},{min:158519,max:220000,rate:.29},{min:220000,max:Infinity,rate:.33}], provincialRate:.11, cpp:{exemption:3500,ceiling:68500,rate:.0595} },
  AU: { currency:"AUD", symbol:"A$", name:"Australia",            flag:"🇦🇺", bands:[{min:0,max:18200,rate:0},{min:18200,max:45000,rate:.19},{min:45000,max:120000,rate:.325},{min:120000,max:180000,rate:.37},{min:180000,max:Infinity,rate:.45}], medicare:.02, medicareThreshold:26000 },
  DE: { currency:"EUR", symbol:"€",  name:"Germany",              flag:"🇩🇪", bands:[{min:0,max:11604,rate:0},{min:11604,max:66761,rate:.14},{min:66761,max:277826,rate:.42},{min:277826,max:Infinity,rate:.45}], socialSecurity:{health:.0735,pension:.093,unemployment:.013,nursing:.017}, solidaritySurcharge:.055 },
  IE: { currency:"EUR", symbol:"€",  name:"Ireland",              flag:"🇮🇪", bands:[{min:0,max:42000,rate:.20},{min:42000,max:Infinity,rate:.40}], usc:[{min:0,max:12012,rate:.005},{min:12012,max:25760,rate:.02},{min:25760,max:70044,rate:.04},{min:70044,max:Infinity,rate:.08}], prsi:.04, taxCredit:3550 },
  NL: { currency:"EUR", symbol:"€",  name:"Netherlands",          flag:"🇳🇱", bands:[{min:0,max:75518,rate:.3693},{min:75518,max:Infinity,rate:.495}], generalCredit:3070 },
  SG: { currency:"SGD", symbol:"S$", name:"Singapore",            flag:"🇸🇬", bands:[{min:0,max:20000,rate:0},{min:20000,max:30000,rate:.02},{min:30000,max:40000,rate:.035},{min:40000,max:80000,rate:.07},{min:80000,max:120000,rate:.115},{min:120000,max:160000,rate:.15},{min:160000,max:200000,rate:.18},{min:200000,max:240000,rate:.19},{min:240000,max:280000,rate:.195},{min:280000,max:320000,rate:.20},{min:320000,max:Infinity,rate:.22}], cpf:.20 },
  AE: { currency:"AED", symbol:"AED ",name:"United Arab Emirates",flag:"🇦🇪", bands:[] },
  PT: { currency:"EUR", symbol:"€",  name:"Portugal",             flag:"🇵🇹", bands:[{min:0,max:7703,rate:.1325},{min:7703,max:11623,rate:.18},{min:11623,max:16472,rate:.23},{min:16472,max:21321,rate:.26},{min:21321,max:27146,rate:.3275},{min:27146,max:39791,rate:.37},{min:39791,max:51997,rate:.435},{min:51997,max:81199,rate:.45},{min:81199,max:Infinity,rate:.48}], socialSecurity:.11 },
};
...
const FX_TO_USD: Record<string, number> = { UK:1.27, US:1.0, CA:0.74, AU:0.66, DE:1.09, IE:1.09, NL:1.09, SG:0.74, AE:0.27, PT:1.09 };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {sub && (
          <div style={{
            fontFamily: "Satoshi, sans-serif", fontSize: 11, fontWeight: 400,
...
// ─── Comparison Chart ─────────────────────────────────────────────────────────
function ComparisonChart({ role, selectedCountry, level }: {
  role: string; selectedCountry: keyof typeof TAX_DATA; level: Level;
}) {
  ...
  return (
    <section style={{
      ...
      <div style={{
        ...
        }}>local currency · ranked by USD equivalent</div>
      </div>
          ...
          {rows.map((r, i) => {
            ...
            return (
              <div key={r.code} style={{
                display: "grid", gridTemplateColumns: "108px 1fr 108px",
                ...
                }}>{r.d.symbol}{r.local.toLocaleString("en")}</div>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div style={{ marginTop: 18, borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: 16 }}>
      <button onClick={() => setOpen(v => !v)} style={{
        ...
        <span style={{
          fontFamily: "Satoshi, sans-serif", fontSize: 14, color: "rgba(255,255,255,0.45)",
      ...
      {open && (
        <div style={{
          ...
          fontWeight: 400, lineHeight: 1.55, color: "rgba(255,255,255,0.45)",
        }}>
          <p style={{ margin: 0 }}>
            Effective rate applies {d.name}&apos;s 2025/26 brackets plus mandatory deductions ({d.currency}).
          </p>
          <p style={{ margin: "8px 0 0" }}>
            Global rank uses USD-equivalent salary across all roles × countries in the dataset.
          </p>
        </div>
      )}
    </div>
...
export default function SalaryCalculator() {
  ...
  const grossLocal = useMemo(() => Math.round(getSalaryForRole(role, country) * LEVEL_MULTIPLIERS[level]), [role, country, level]);
  ...
  const fmtMoney = (n: number) => `${sym}${Math.round(n).toLocaleString("en")}`;
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col">
      ...
      <main className="flex-1" style={{ padding: "24px 24px 80px" }}>
        <div style={{
          ...
          <aside style={{
            ...
            <div>
              <div style={{
                ...
                fontSize: 16, letterSpacing: "0.02em", color: "#ffffff",
              }}>Origio</div>
              <div style={{
                fontFamily: "Satoshi, sans-serif", fontSize: 11,
                fontWeight: 400, color: "rgba(255,255,255,0.45)", marginTop: 4,
                  ...
                  return (
                    <button key={c} onClick={() => setCountry(c)}
                      style={{
                        ...
                        borderLeftColor: active ? "#4de6cc" : "transparent",
              ...
              <div ref={roleRef} style={{ position: "relative" }}>
                ...
                >
                  ...
                  <span style={{
                    ...
                    transition: "transform 160ms ease", display: "inline-block", lineHeight: 1,
                  }}>⌄</span>
                </button>
                {roleOpen && (
                  <div style={{
                    ...
                    background: "#111111", border: "1px solid rgba(255,255,255,0.07)",
                    borderRadius: 10, maxHeight: 280, overflowY: "auto", zIndex: 20, padding: 4,
                  }}>
                    {ROLES.map(r => {
                      const active = r === role;
                      return (
                        <button key={r} onClick={() => { setRole(r); setRoleOpen(false); }} style={{
                          ...
                          padding: "9px 12px", background: active ? "#1c1c1c" : "transparent",
          ...
          <section style={{ display: "flex", flexDirection: "column", gap: 28, minWidth: 0 }}>
            ...
            <div>
              ...
              <div style={{
                ...
                <span style={{
                  fontFamily: "Satoshi, sans-serif", fontSize: "0.35em",
                  fontWeight: 500, color: "rgba(255,255,255,0.45)",
                  verticalAlign: "0.55em", marginRight: 8, letterSpacing: 0,
                }}>{sym}</span>
                <AnimatedNumber value={grossLocal} format={(n) => Math.round(n).toLocaleString("en")} />
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              ...
              <StatTile
                ...
                value={effectiveRate}
                sub={result?.items?.map(i => i.label).join(" · ")}
                blurred={false}
                format={fmtPct}
              />
            </div>
          ...
          <aside style={{
            ...
            <div style={{
              fontFamily: "Satoshi, sans-serif", fontSize: 12, fontWeight: 400,
            ...
            <BenchmarkRow
              ...
              sub={isPro
                ? `${sym}${bench.rentMonthly.toLocaleString("en")} / mo · 1-bed, major city`
