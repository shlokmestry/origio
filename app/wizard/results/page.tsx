  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-bold font-mono" style={{ color }}>{value.toFixed(1)}/10</span>
      </div>
      ...
      <p className="text-[9px] text-[#444] font-mono">{desc}</p>
    </div>
  );
}

function EmailCapture({ topCountry, topCountryFlag, matchPercent, jobRole, grossSalary, netMonthly, taxRate, rentCost, visaLabel, currency }: {
  topCountry: string; topCountryFlag: string; matchPercent: number; jobRole: string;
  grossSalary: number; netMonthly: number; taxRate: number; rentCost: number; visaLabel: string; currency: string;
}) {
  ...
  if (submitted) {
    return (
      <div className="border border-[#2a2a2a] bg-[#0d0d0d] p-6 mt-8">
        <p className="text-sm font-bold text-accent uppercase tracking-widest">✓ Check your inbox</p>
      </div>
    );
  }

  return (
    <div className="border border-[#2a2a2a] bg-[#0d0d0d] p-6 mt-8 space-y-4" style={{ boxShadow: "4px 4px 0 #1a1a1a" }}>
      <div>
        <p className="text-[10px] font-bold text-[#888880] uppercase tracking-[0.2em] mb-2">Save your results</p>
        <p className="font-heading text-lg font-extrabold text-[#f0f0e8] uppercase tracking-tight">
          {topCountryFlag} {topCountry} breakdown by email
        </p>
        <p className="text-xs text-[#666660] mt-1">Salary, tax, and cost breakdown for your results. No spam. One email.</p>
      </div>
      <div className="flex flex-col sm:flex-row gap-3">
        <input type="email" placeholder="your@email.com" value={email}
          onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSubmit()}
          className="flex-1 bg-[#111] border border-[#2a2a2a] px-4 py-3 text-sm text-[#f0f0e8] placeholder-[#444] focus:outline-none focus:border-accent transition-colors" />
        <button onClick={handleSubmit} disabled={loading}
          className="px-6 py-3 text-[11px] font-extrabold uppercase tracking-widest bg-accent text-[#0a0a0a] disabled:opacity-50 flex-shrink-0"
...
function ScoreCard({ match, answers, jobRoleDef, rank, excludedCountries }: {
  match: CountryMatch; answers: Partial<WizardAnswers>; jobRoleDef: typeof JOB_ROLES[0] | undefined;
  rank: number; excludedCountries: { name: string; reason: string }[];
}) {
  ...
  return (
    <div className="border border-[#2a2a2a] bg-[#0d0d0d] p-5 space-y-5 h-fit">
      <div>
        <p className="text-[9px] font-bold text-[#444] uppercase tracking-[0.2em] mb-2">Score breakdown</p>
        <p className="text-[11px] text-[#666660] leading-relaxed font-mono">{summary}</p>
      </div>
      <div className="space-y-3 border-t border-[#1a1a1a] pt-4">
        {Object.values(scores).map((s) => <ScoreBar key={s.label} {...s} />)}
      </div>
      {excludedCountries.length > 0 && (
        <div className="border-t border-[#1a1a1a] pt-4">
          <p className="text-[9px] font-bold text-[#333] uppercase tracking-widest mb-3">Filtered out</p>
...
function ScoreToggle({ match, answers, jobRoleDef, rank }: {
  ...
  return (
    <div className="mt-3">
      <button onClick={(e) => { e.preventDefault(); setOpen(!open); }}
        className="flex items-center gap-1.5 text-[10px] font-bold text-[#333] hover:text-[#888880] transition-colors uppercase tracking-widest">
      ...
      {open && (
        <div className="mt-3 pt-3 border-t border-[#1a1a1a] space-y-3">
          <p className="text-[10px] text-[#666660] leading-relaxed font-mono">{summary}</p>
          <div className="space-y-2.5">{Object.values(scores).map((s) => <ScoreBar key={s.label} {...s} />)}</div>
        </div>
      )}
    </div>
  );
...
function computeExcluded(matchSlugs: string[], answers: Partial<WizardAnswers>): { name: string; reason: string }[] {
  ...
  const SLUG_TO_NAME: Record<string, string> = {
    ...
    "ireland":"Ireland","united-kingdom":"United Kingdom","netherlands":"Netherlands","france":"France",
    "finland":"Finland","belgium":"Belgium","denmark":"Denmark","austria":"Austria","italy":"Italy",
    "usa":"USA","japan":"Japan","india":"India","uae":"UAE","brazil":"Brazil","malaysia":"Malaysia",
...
function TakeHomeCard({ match, jobRoleDef, isPro }: {
  ...
  const Row = ({ label, value, color, border = true }: { label: string; value: string; color?: string; border?: boolean }) => (
    <div className={`flex items-center justify-between py-3 ${border ? "border-b border-[#1a1a1a]" : ""}`}>
      <span className="text-[11px] font-bold text-[#888880] uppercase tracking-widest">{label}</span>
      <span className="font-mono text-sm font-bold" style={{ color: color ?? "#f0f0e8" }}>{value}</span>
    </div>
  );

  return (
    <div className="mt-8 border border-[#2a2a2a]" style={{ boxShadow: "4px 4px 0 #2a2a2a" }}>
      <div className="flex items-center justify-between px-5 py-3 border-b border-[#2a2a2a] bg-[#0d0d0d]">
        ...
        <span className="text-[10px] font-mono text-[#444]">{match.country.data.incomeTaxRateMid}% tax</span>
      </div>
      <div className="p-5 bg-[#0a0a0a]">
        <Row label="Gross salary" value={`${cs}${gross.toLocaleString()}/yr`} />
        <Row label={`Income tax (${match.country.data.incomeTaxRateMid}%)`} value={`-${cs}${taxAmount.toLocaleString()}`} color="#ef4444" />
        <div className="h-px bg-[#2a2a2a] my-1" />
        <Row label="Net annual" value={`${cs}${netAnnual.toLocaleString()}/yr`} color="#00ffd5" />
        <Row label="Net monthly" value={`${cs}${netMonthly.toLocaleString()}/mo`} color="#00ffd5" />

        <div className="relative">
          <div className={isPro ? "" : "blur-sm pointer-events-none select-none"}>
            <Row label="Rent + groceries + transport" value={`-${cs}${totalCosts.toLocaleString()}/mo`} color="#ef4444" />
            <Row label="Disposable income" value={`${cs}${disposable.toLocaleString()}/mo`} color={disposable > 0 ? "#4ade80" : "#ef4444"} border={false} />
          </div>
          {!isPro && (
            <div className="absolute inset-0 flex items-center justify-center bg-[#0a0a0a]/60">
              <Link href="/pro" className="flex items-center gap-2 px-4 py-2 text-[10px] font-extrabold uppercase tracking-widest bg-accent text-[#0a0a0a]"
                style={{ boxShadow: "2px 2px 0 #00aa90" }}>
                <Lock className="w-3 h-3" /> Full breakdown — Pro
              </Link>
            </div>
          )}
        </div>
        <p className="text-[9px] text-[#333] mt-4 leading-relaxed font-mono">
...
export default function WizardResultsPage() {
  ...
  useEffect(() => {
    ...
    try { setMatches(JSON.parse(raw)); if (answersRaw) setAnswers(JSON.parse(answersRaw)); }
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center px-6">
        <div className="w-full max-w-sm space-y-8">
          <div className="mx-auto w-16 h-16 border border-[#2a2a2a] flex items-center justify-center">
            <Globe2 className="w-8 h-8 text-accent" style={{ animation: "spin 3s linear infinite" }} />
          </div>
          <div className="space-y-1">
            <h2 className="font-heading text-2xl font-extrabold text-[#f0f0e8] uppercase tracking-tight">Ranking 25 countries</h2>
            <p key={loadingStep} className="text-[#666660] text-sm font-mono">{LOADING_STEPS[loadingStep]}</p>
          </div>
          <div className="w-full h-px bg-[#1a1a1a]">
            <div className="h-full bg-accent transition-all duration-100" style={{ width: loadingProgress + "%" }} />
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <section className="pt-16 pb-14 border-b border-[#1a1a1a]">
          <div className="grid lg:grid-cols-[1fr_300px] gap-12 items-start">
            <div>
              ...
              <div className="flex items-start gap-5 mb-6">
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-2.5 mt-4">
                    <span className="font-heading text-5xl font-extrabold" style={{ color: pctColor }}>{top.matchPercent}%</span>
              ...
              {topSalary && (
                <div className="flex flex-wrap gap-x-5 gap-y-1 mb-6 text-[12px] font-mono text-[#666660]">
              ...
              {top.reasons.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-8">
                  {top.reasons.map(r => (
                    <span key={r} className="text-[10px] font-bold px-3 py-1 uppercase tracking-widest"
                      style={{ border: `1px solid ${pctColor}40`, color: pctColor }}>{r}</span>
                  ))}
                </div>
              )}

              <div className="flex flex-wrap items-center gap-4">
                <Link href={"/country/" + top.country.slug + "/personalised"}
                  className="px-7 py-3.5 text-[11px] font-extrabold uppercase tracking-[0.15em] bg-accent text-[#0a0a0a]"
                  style={{ boxShadow: "3px 3px 0 #f0f0e8", border: "2px solid #f0f0e8" }}>
                  Full report →
                </Link>
                <button onClick={handleViewOnGlobe}
                  className="px-7 py-3.5 text-[11px] font-extrabold uppercase tracking-[0.15em] border border-[#2a2a2a] text-[#888880] hover:text-[#f0f0e8] hover:border-[#444] transition-colors">
                  See on globe
                </button>
                <button onClick={() => router.push("/wizard")}
                  className="text-[11px] font-bold text-[#444] hover:text-[#f0f0e8] transition-colors uppercase tracking-widest ml-auto">
                  Retake →
                </button>
              </div>
              ...
              {!user && (
                <EmailCapture topCountry={top.country.name} topCountryFlag={top.country.flagEmoji}
                  matchPercent={top.matchPercent} jobRole={jobRoleDef?.label ?? ""}
                  grossSalary={grossSalary} netMonthly={netMonthly}
                  taxRate={top.country.data.incomeTaxRateMid} rentCost={top.country.data.costRentCityCentre}
                  visaLabel={getVisaLabel(top.country.data.visaDifficulty)} currency={cs} />
              )}
            </div>

            <div className="lg:sticky lg:top-6">
              <ScoreCard match={top} answers={answers} jobRoleDef={jobRoleDef} rank={1} excludedCountries={excludedCountries} />
            </div>
          </div>
        </section>
        ...
        <section className="py-14 border-b border-[#1a1a1a]">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {matches.slice(0, 3).map((m, i) => {
              ...
              return (
                <div key={m.country.slug} className="border border-[#1a1a1a] p-5 hover:border-[#2a2a2a] transition-colors"
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="font-mono text-lg font-bold" style={{ color: mPct }}>{m.matchPercent}%</span>
                  </div>
                  <div className="text-3xl mb-3">{m.country.flagEmoji}</div>
                  <p className="font-heading text-base font-extrabold uppercase tracking-tight mb-1">{m.country.name}</p>
                  {salary && (
                    <p className="text-[10px] text-[#666660] font-mono">
                  ...
                  <div className="mt-3 pt-3 border-t border-[#1a1a1a] flex items-center justify-between">
                    <p className="text-[10px] text-[#666660] font-mono">{mcs}{m.country.data.costRentCityCentre.toLocaleString()}/mo rent</p>
          ...
          {isPro && (
            <div className="mt-6">
              <Link href={compareHref} className="inline-flex items-center gap-2 text-[11px] font-bold text-[#888880] hover:text-accent transition-colors uppercase tracking-widest">
                Compare top 3 side by side <ArrowRight className="w-3 h-3" />
              </Link>
            </div>
          )}
        </section>
          ...
          <div className="grid sm:grid-cols-2 gap-px bg-[#1a1a1a]">
            {[
              ...
              <div key={item.label} className="bg-[#0a0a0a] px-6 py-5">
                ...
                <p className="text-sm font-mono font-bold text-[#f0f0e8]">{item.value}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="py-14 pb-20">
          <div className="flex items-baseline justify-between mb-8">
            ...
            <p className="text-[10px] font-mono text-[#888880]">{visibleMatches.length} / 25</p>
          </div>
              ...
              return (
                <div key={match.country.slug} className="border-b border-[#111111]"
                  ...
                  <Link href={"/country/" + match.country.slug + "/personalised"}
                    className="flex items-center gap-4 py-4 hover:bg-[#0d0d0d] transition-colors px-3 -mx-3"
                    style={isTop3 ? { paddingLeft: "14px", marginLeft: "-2px" } : {}}>
                    <span className="font-mono text-[11px] font-bold w-6 text-right flex-shrink-0"
                      style={{ color: isTop3 ? RANK_COLORS[i] : "#2a2a2a" }}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <span className="text-xl flex-shrink-0">{match.country.flagEmoji}</span>
                    <div className="flex-1 min-w-0">
                      <p className="font-heading text-[14px] font-extrabold uppercase tracking-tight text-[#f0f0e8] truncate">{match.country.name}</p>
                      {salary && (
                    ...
                    <span className="font-mono text-[13px] font-bold flex-shrink-0" style={{ color: matchPercentColor(match.matchPercent) }}>
                      {match.matchPercent}%
                    </span>
                  </Link>
                  {i >= 3 && (
            ...
            <div className="mt-0">
              <div className="relative overflow-hidden" style={{ height: 100 }}>
                {[visibleMatches.length + 1, visibleMatches.length + 2].map((n, i) => (
                  <div key={n} className="flex items-center gap-4 py-4 border-b border-[#111111] px-3"
                    ...
                    <span className="font-mono text-[11px] font-bold w-6 text-right text-[#2a2a2a]">{String(n).padStart(2, "0")}</span>
              ...
              <div className="flex items-center justify-between pt-5 border-t border-[#1a1a1a]">
                <div>
                  ...
                  <p className="text-[10px] text-[#888880] mt-0.5 uppercase tracking-widest font-mono">Full ranking — €19.99 once</p>
                </div>
                <Link href="/pro"
                  ...
                  style={{ boxShadow: "2px 2px 0 #f0f0e8", border: "2px solid #f0f0e8" }}>
                  Get Pro
                </Link>
              </div>
            </div>
          ...
          {isPro && matches.length >= 3 && (
            <div className="mt-8 pt-6 border-t border-[#1a1a1a] flex items-center justify-between">
              <p className="text-[10px] text-[#888880] uppercase tracking-widest font-mono">All 25 countries ranked</p>
              <Link href={compareHref} className="inline-flex items-center gap-2 text-[11px] font-bold text-[#888880] hover:text-accent transition-colors uppercase tracking-widest">
