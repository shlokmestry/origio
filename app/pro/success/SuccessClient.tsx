...
function Tile({ char, state }: { char: string; state: TileState }) {
  ...
  return (
    <span style={{
      ...
      overflow:       'hidden',
      flexShrink:     0,
      transition:     'color 0.06s, background 0.1s',
      userSelect:     'none',
    }}>
      {char === ' ' ? '\u00a0' : char}
      {/* centre crease */}
...
// ── Tile strip (DESTINATION or MATCH columns) ─────────────────────────────────
...
function useTileEngine() {
  ...
  // Verifying: random cycling chars
  ...
  const startSuccess = useCallback((dest: string, match: string) => {
    ...
    function animate(
      ...
      const chars = [...target]
      const initial: TileData[] = chars.map(() => ({ char: rnd(), state: 'cy' }))
      setter(initial)
...
function StatusBadge({ mode }: { mode: 'confirmed' | 'searching' | 'failed' | 'none' }) {
  ...
  const colors = {
    confirmed: '#4de6cc',
    searching: 'rgba(255,255,255,0.38)',
    failed:    '#ff4f4f',
  }
  ...
  return (
    <span style={{
      ...
      <span style={{
        ...
        animation: mode === 'confirmed' ? 'breathe 2s ease-in-out infinite'
                 : mode === 'searching' ? 'pulse 1.1s ease-in-out infinite'
                 : 'none',
      }} />
      {labels[mode]}
    </span>
...
function DepartureBoard({
  destTiles, matchTiles, statusMode, glowing,
}: {
  destTiles:   TileData[]
  matchTiles:  TileData[]
  statusMode:  'confirmed' | 'searching' | 'failed' | 'none'
  glowing:     boolean
}) {
  return (
    <div style={{
      ...
      position:     'relative',
      boxShadow:    glowing
        ? '0 0 60px rgba(77,230,204,0.07), 0 0 0 1px rgba(77,230,204,0.14)'
        : 'none',
    }}>
      {/* dot-grid texture */}
      <div style={{
...
export default function SuccessClient() {
  ...
  // Start verifying animation immediately
  useEffect(() => {
    setMounted(true)
    startVerifying()
  }, [startVerifying])
      ...
      try {
        ...
        if (data.paid && data.pro) {
          ...
          const { data: result } = await supabase
            ...
            .maybeSingle()
          const match = result?.top_countries?.[0] ?? null
          if (match) setTopMatch(match)
          goSuccess(match)
          return
        }
    ...
    <div style={{
      ...
      <style>{`
        ...
        @media (max-width: 700px) {
          .board-grid { grid-template-columns: 1fr 100px 140px !important; }
      ...
      <div className="page-pad" style={{ maxWidth: 1040, margin: '0 auto', padding: '104px 40px 120px' }}>
        ...
        {/* ── DEPARTURE BOARD (always visible) ── */}
        <DepartureBoard
          destTiles={destTiles}
          ...
          <div className="success-grid" style={{
            ...
            <div>
              ...
              <div className="fu d2" style={{
                ...
                background:  'rgba(77,230,204,0.10)',
                border:      '1px solid rgba(77,230,204,0.22)',
                borderRadius: 100,
                padding:     '6px 14px',
                marginBottom: 28,
              }}>
              ...
              <h1 className="fu d3" style={{
                ...
                fontWeight:    400, lineHeight: 0.90,
                letterSpacing: '-0.02em',
              ...
              <div className="fu d5" style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
                <Link href="/wizard" style={{
                  ...
                  textDecoration: 'none',
                  boxShadow:      '3px 3px 0 rgba(77,230,204,0.3)',
                }}>
                  Run quiz — see all 25
                  <ArrowRight size={13} />
                </Link>
            ...
            <div className="fu d4">
              ...
              {PRO_FEATURES.map((f, i) => (
                <div key={f.n} style={{
                  ...
                  paddingTop:   i === 0 ? 0 : 18,
                  paddingBottom: i < PRO_FEATURES.length - 1 ? 18 : 0,
                  borderBottom:  i < PRO_FEATURES.length - 1
