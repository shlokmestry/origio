/** Maps country slug to ISO 3166-1 alpha-2 code for use with flag-icons */
export const SLUG_TO_ISO: Record<string, string> = {
  'france': 'fr',
  'germany': 'de',
  'united-states': 'us',
  'united-kingdom': 'gb',
  'canada': 'ca',
  'australia': 'au',
  'portugal': 'pt',
  'spain': 'es',
  'italy': 'it',
  'netherlands': 'nl',
  'sweden': 'se',
  'norway': 'no',
  'denmark': 'dk',
  'finland': 'fi',
  'switzerland': 'ch',
  'austria': 'at',
  'ireland': 'ie',
  'poland': 'pl',
  'belgium': 'be',
  'singapore': 'sg',
  'japan': 'jp',
  'south-korea': 'kr',
  'thailand': 'th',
  'vietnam': 'vn',
  'malaysia': 'my',
  'indonesia': 'id',
  'philippines': 'ph',
  'india': 'in',
  'china': 'cn',
  'uae': 'ae',
  'saudi-arabia': 'sa',
  'brazil': 'br',
  'mexico': 'mx',
  'argentina': 'ar',
  'colombia': 'co',
  'chile': 'cl',
  'new-zealand': 'nz',
  'south-africa': 'za',
  'nigeria': 'ng',
  'kenya': 'ke',
  'ghana': 'gh',
  'egypt': 'eg',
  'morocco': 'ma',
  'georgia': 'ge',
  'ukraine': 'ua',
  'czech-republic': 'cz',
  'hungary': 'hu',
  'romania': 'ro',
  'croatia': 'hr',
  'estonia': 'ee',
  'serbia': 'rs',
  'cyprus': 'cy',
  'latvia': 'lv',
  'lithuania': 'lt',
  'slovakia': 'sk',
  'slovenia': 'si',
  'bulgaria': 'bg',
  'greece': 'gr',
  'turkey': 'tr',
  'israel': 'il',
  'panama': 'pa',
  'costa-rica': 'cr',
  'taiwan': 'tw',
  'pakistan': 'pk',
  // Passport-power extended set
  'luxembourg': 'lu',
  'malta': 'mt',
  'iceland': 'is',
  'usa': 'us',
  'monaco': 'mc',
  'andorra': 'ad',
  'liechtenstein': 'li',
  'czechia': 'cz',
  'san-marino': 'sm',
  'brunei': 'bn',
  'barbados': 'bb',
  'the-bahamas': 'bs',
  'st-vincent-grenadines': 'vc',
  'st-kitts-nevis': 'kn',
  'uruguay': 'uy',
  'seychelles': 'sc',
  'antigua-barbuda': 'ag',
  'vatican-city': 'va',
  'mauritius': 'mu',
  'grenada': 'gd',
  'paraguay': 'py',
  'dominica': 'dm',
  'trinidad-tobago': 'tt',
  'st-lucia': 'lc',
  'macao': 'mo',
  'peru': 'pe',
  'russia': 'ru',
  'qatar': 'qa',
  'kuwait': 'kw',
  'bahrain': 'bh',
  'oman': 'om',
  'belize': 'bz',
  'ecuador': 'ec',
  'maldives': 'mv',
  'timor-leste': 'tl',
  'guyana': 'gy',
  'fiji': 'fj',
  'vanuatu': 'vu',
  'jamaica': 'jm',
  'nauru': 'nr',
  'papua-new-guinea': 'pg',
  'botswana': 'bw',
  'kosovo': 'xk',
  'belarus': 'by',
  'bolivia': 'bo',
  'kazakhstan': 'kz',
  'suriname': 'sr',
  'namibia': 'na',
  'lesotho': 'ls',
  'eswatini': 'sz',
  'dominican-republic': 'do',
  'malawi': 'mw',
  'rwanda': 'rw',
  'tanzania': 'tz',
  'azerbaijan': 'az',
  'the-gambia': 'gm',
  'tunisia': 'tn',
  'uganda': 'ug',
  'armenia': 'am',
  'benin': 'bj',
  'mongolia': 'mn',
  'zambia': 'zm',
  'cape-verde-islands': 'cv',
  'sierra-leone': 'sl',
  'zimbabwe': 'zw',
  'kyrgyzstan': 'kg',
  'mozambique': 'mz',
  'sao-tome-principe': 'st',
  'uzbekistan': 'uz',
  'burkina-faso': 'bf',
  'cuba': 'cu',
  'gabon': 'ga',
  'madagascar': 'mg',
  'togo': 'tg',
  'algeria': 'dz',
  'cote-divoire': 'ci',
  'mauritania': 'mr',
  'senegal': 'sn',
  'equatorial-guinea': 'gq',
  'niger': 'ne',
  'guinea': 'gn',
  'mali': 'ml',
  'tajikistan': 'tj',
  'chad': 'td',
  'comoro-islands': 'km',
  'guinea-bissau': 'gw',
  'jordan': 'jo',
  'angola': 'ao',
  'burundi': 'bi',
  'cameroon': 'cm',
  'central-african-republic': 'cf',
  'haiti': 'ht',
  'liberia': 'lr',
  'bhutan': 'bt',
  'cambodia': 'kh',
  'congo-rep': 'cg',
  'djibouti': 'dj',
  'laos': 'la',
  'congo-dem-rep': 'cd',
  'turkmenistan': 'tm',
  'myanmar': 'mm',
  'ethiopia': 'et',
  'lebanon': 'lb',
  'south-sudan': 'ss',
  'sudan': 'sd',
  'libya': 'ly',
  'sri-lanka': 'lk',
  'eritrea': 'er',
  'iran': 'ir',
  'palestinian-territory': 'ps',
  'bangladesh': 'bd',
  'north-korea': 'kp',
  'nepal': 'np',
  'somalia': 'so',
  'yemen': 'ye',
  'iraq': 'iq',
  'syria': 'sy',
  'afghanistan': 'af',
  'hong-kong': 'hk',
  'moldova': 'md',
  'venezuela': 've',
  'north-macedonia': 'mk',
  'montenegro': 'me',
  'albania': 'al',
  'bosnia-herzegovina': 'ba',
  'micronesia': 'fm',
  'palau-islands': 'pw',
  'kiribati': 'ki',
  'tuvalu': 'tv',
  'marshall-islands': 'mh',
  'tonga': 'to',
  'samoa': 'ws',
  'solomon-islands': 'sb',
  'nicaragua': 'ni',
  'honduras': 'hn',
  'el-salvador': 'sv',
  'guatemala': 'gt',
}

/** Get ISO alpha-2 code from a country slug, or undefined if not mapped */
export function slugToIso(slug: string): string | undefined {
  return SLUG_TO_ISO[slug]
}

/** Maps city slug to ISO 3166-1 alpha-2 country code for flag icons */
export const CITY_SLUG_TO_ISO: Record<string, string> = {
  lisbon: 'pt', porto: 'pt', funchal: 'pt',
  london: 'gb', manchester: 'gb', edinburgh: 'gb', bristol: 'gb', glasgow: 'gb',
  dublin: 'ie', cork: 'ie',
  amsterdam: 'nl', rotterdam: 'nl', eindhoven: 'nl', utrecht: 'nl', 'the-hague': 'nl',
  berlin: 'de', munich: 'de', hamburg: 'de', cologne: 'de', frankfurt: 'de',
  barcelona: 'es', madrid: 'es', valencia: 'es', malaga: 'es', seville: 'es', bilbao: 'es',
  paris: 'fr',
  milan: 'it', rome: 'it', florence: 'it',
  athens: 'gr',
  vienna: 'at',
  prague: 'cz',
  budapest: 'hu',
  bucharest: 'ro',
  warsaw: 'pl',
  stockholm: 'se',
  copenhagen: 'dk',
  helsinki: 'fi',
  oslo: 'no',
  brussels: 'be',
  zurich: 'ch',
  limassol: 'cy',
  split: 'hr',
  belgrade: 'rs',
  tbilisi: 'ge',
  tallinn: 'ee',
  'new-york': 'us', 'san-francisco': 'us', austin: 'us', miami: 'us',
  toronto: 'ca', vancouver: 'ca', montreal: 'ca', calgary: 'ca',
  medellin: 'co', 'buenos-aires': 'ar', 'sao-paulo': 'br',
  'mexico-city': 'mx', 'panama-city': 'pa', 'san-jose-cr': 'cr',
  singapore: 'sg',
  tokyo: 'jp', osaka: 'jp', kyoto: 'jp', fukuoka: 'jp',
  sydney: 'au', melbourne: 'au', brisbane: 'au', perth: 'au', auckland: 'nz',
  seoul: 'kr',
  dubai: 'ae', 'abu-dhabi': 'ae',
  bangkok: 'th', 'chiang-mai': 'th',
  bali: 'id',
  'kuala-lumpur': 'my',
  'da-nang': 'vn', 'ho-chi-minh-city': 'vn',
  bangalore: 'in',
  'cape-town': 'za',
}

/** Get ISO alpha-2 code from a city slug, or undefined if not mapped */
export function citySlugToIso(slug: string): string | undefined {
  return CITY_SLUG_TO_ISO[slug]
}
